...
(function(global){
	var 
		...
		docScripts = oDOC.getElementsByTagName("script"),
		...
		is_opera = fOBJTOSTRING.call(global.opera) == "[object Opera]",
		...
		is_script_async = (oDOC.createElement("script").async === true),
		...
		global_defs = {
			...
			xhr:1, // use XHR trick to preload local scripts
			dupe:1, // allow duplicate scripts? defaults to true now 'cause is slightly more performant that way (less checks)
			base:"", // base path to prepend to all non-absolute-path scripts
			which:"head" // which DOM object ("head" or "body") to append scripts to
		}
	;
	global_defs.preserve = 0; // force preserve execution order of all loaded scripts (regardless of preloading)
	global_defs.preload = 1; // use various tricks for "preloading" scripts
	
	append_to.head = oDOC.head || oDOC.getElementsByTagName("head");
	append_to["body"] = oDOC.getElementsByTagName("body");
	...
	function scriptTagExists(uri) { // checks if a script uri has ever been loaded into this page's DOM
		...
		while (script = docScripts[++idx]) {
			if (typeof script.src == "string" && uri === canonicalScriptURI(script.src) && script.type !== "script/cache") return 1;
		}
		return 0;
	}
	function engine(queueExec,opts) {
		...
		function handleXHRPreload(xhr,scriptentry) {
			if (xhr.readyState === 4) {
				xhr.onreadystatechange = fNOOP; // fix a memory leak in IE
				scriptentry["preloaddone"] = 1;
		...
		function createScriptTag(scriptentry,src,type,charset,onload,scriptText) {
			...
			setTimeout(function() { // this setTimeout waiting "hack" prevents a nasty race condition browser hang (IE) when the document.write("<script defer=true>") type dom-ready hack is present in the page
				...
				append_to[_script_which].insertBefore(scriptElem,(_script_which==="head"?append_to[_script_which].firstChild:nNULL));
				if (typeof scriptText == "string") { // script text already avaiable from XHR preload, so just inject it
					scriptElem.text = scriptText;
					handleScriptLoad(scriptElem,scriptentry,1); // manually call 'load' callback function, skipReadyCheck=true
				}
			},0);
		}
		function loadScriptElem(scriptentry,src,type,charset) {
			all_scripts[scriptentry.srcuri] = 1;
			createScriptTag(scriptentry,src,type,charset,handleScriptLoad);
		}
		function loadScriptCache(scriptentry,src,type,charset) {
			var args = arguments;
			if (first_pass && scriptentry["preloaddone"] == nNULL) { // need to preload into cache
				scriptentry["preloaddone"] = 0;
			...
			else if (!first_pass && scriptentry["preloaddone"] != nNULL && !scriptentry["preloaddone"]) { // preload still in progress, make sure trigger is set for execution later
		...
		function loadScriptXHR(scriptentry,src,type,charset) {
			...
			if (first_pass && scriptentry["preloaddone"] == nNULL) { // need to preload
				scriptentry["preloaddone"] = 0;
			...
			else if (!first_pass && scriptentry["preloaddone"] != nNULL && !scriptentry["preloaddone"]) {	// preload XHR still in progress, make sure trigger is set for execution later
		...
		function loadScript(o) {
			...
			if (!o) return; // skip over this script call if there's nothing to load

			if (o.allowDup == nNULL) o.allowDup = opts.dupe;

			if (typeof charset != "string") charset = nNULL;
			allowDup = !(!allowDup);
			if (!allowDup && 
				(
				...
				if (scripts[src_uri] != nNULL && scripts[src_uri]["preloaddone"] && !scripts[src_uri]["done"] && same_domain) {
					// this script was preloaded via XHR, but is a duplicate, and dupes are not allowed
					handleScriptLoad(nNULL,scripts[src_uri],1); // mark the entry as done and check if chain group is done
				}
				return;
			}
			if (scripts[src_uri] == nNULL) scripts[src_uri] = {};
			scriptentry = scripts[src_uri];
			if (scriptentry.which == nNULL) scriptentry.which = _which;
			scriptentry["done"] = 0;
		...
		publicAPI = {
			script:function() {
					...
					for (idx=-1; args[++idx];) {
						...
						if (idx===0) {
							queueAndExecute(function(){
								loadScript((typeof args[0] == "string") ? {src:args[0]} : args[0]);
							});
						}
				...
				end_of_chain_check_interval = setTimeout(function(){first_pass = 0;},5); // hack to "detect" the end of the chain if a wait() is not the last call
				return use_engine;
			},
			wait:function(func) {
				fCLEARTIMEOUT(end_of_chain_check_interval);
	...
	function processOpts(opts) {
		...
		for (k in allOpts) {
			if (allOpts.hasOwnProperty(k) && global_defs[allOpts[k]] != nNULL) newOpts[allOpts[k]] = (opts[k] != nNULL) ? opts[k] : global_defs[allOpts[k]];
		}
		...
		if (!newOpts["preload"]) newOpts.cache = newOpts.order = newOpts.xhr = 0; // turn off all flags if preloading is disabled
		newOpts.which = (newOpts.which === "head" || newOpts.which === "body") ? newOpts.which : "head";
		return newOpts;
	}
	...
	global.$LAB = {
		...
		setOptions:function(opts){ // set options per chain
			return engine(0,processOpts(opts));
		},
	...
	*/ 
	(function(addEvent,domLoaded,handler){
		if (oDOC.readyState == nNULL && oDOC[addEvent]){
			oDOC.readyState = "loading";
			oDOC[addEvent](domLoaded,handler = function(){
				oDOC.removeEventListener(domLoaded,handler,0);
